var EdicionConceptos_cotizacion = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Concepto de Cotizacion';
	...
	this.configurarComboConcepto=function(){
		...
		$('select[name="fk_concepto"]').wijcombobox({			
		 ...
		 $('.contenedor_fk_concepto input[role="textbox"]').bind('focus', function(){			
			if (me.conceptoEnAjax) return true;			
			me.setDSConcepto();
			me.conceptoEnAjax=true;
		 });
	};
		
		
	this.setDSConcepto = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+'conceptos_cotizacion/buscarConcepto',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.setDSUnidad = function(){		
		...
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+'conceptos_cotizacion/buscarUnidad',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.configurarComboCotizacion=function(){
		var me=this;
		
		$('select[name="fk_cotizacion"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:true,			
		 ...
		 $('.contenedor_fk_cotizacion input[role="textbox"]').bind('focus', function(){			
			if (me.cotizacionEnAjax) return true;			
			me.setDSCotizacion();
			me.cotizacionEnAjax=true;
		 });
	};
		
		
	this.setDSCotizacion = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+'conceptos_cotizacion/buscarCotizacion',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_cotizacion"]').wijcombobox('option','data',datasource);
	};
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Concepto: '+getValorCampo('fk_concepto'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_cotizacion"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_cotizacion"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_cotizacion'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_cotizacion'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_cotizacion'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboCotizacion();
	};
